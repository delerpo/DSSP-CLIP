    ...
    with torch.no_grad():
        ...
        for data in test_dataloader:
            ...
            if torch.sum(labels) >= 1:
                ...
                categories = data[3]
                _, anomaly_maps = clip_model.detect_forward(imgs, args, categories=categories)
...
def compute_pro(masks: ndarray, amaps: ndarray, num_th: int = 200) -> float:
    ...
    for th in np.arange(min_th, max_th, delta):
        ...
        current_pro = mean(pros) if pros else 0.0
        results.append({"pro": current_pro, "fpr": fpr, "threshold": th})
    ...
    # TODO: 确保按 FPR 排序，否则 AUC 计算可能出错（如果是负值）
...
def get_res_str(metrics, row_name):
    """
    格式化单行输出
    """
    # 安全取值，防止 Key Error
    s_metric = metrics.get('Sample_CLS', {})
    p_metric = metrics.get('Pixel', {})

    s_auc = s_metric.get('AUROC', 0)
    s_ap  = s_metric.get('AP', 0)
    s_f1  = s_metric.get('max-F1', 0)

    p_auc = p_metric.get('AUROC', 0)
    p_ap  = p_metric.get('AP', 0)
    p_f1  = p_metric.get('max-F1', 0)
    p_pro = p_metric.get('PRO', 0)

    # 格式化字符串
    row_fmt = "{:<15} | {:<10.4f} {:<10.4f} {:<10.4f} | {:<10.4f} {:<10.4f} {:<10.4f} {:<10.4f}"
    return row_fmt.format(row_name, s_auc, s_ap, s_f1, p_auc, p_ap, p_f1, p_pro)
...
def evaluation_pixel(clip_model:CLIP, dataset_name, dataloader, args, device):
    ...
    with torch.no_grad():
        for items in tqdm(dataloader):
            imgs, labels, gt, categories = items[:4]
            imgs = imgs.to(device)
            predict_labels, predict_masks = clip_model.detect_forward(imgs, args, categories=categories)
            ...
            # Binarize ground-truth and collect pixel-level labels
            gt[gt > 0.5] = 1
            gt[gt <= 0.5] = 0
...
def eval_all_class(clip_model: CLIP, dataset_name, test_dataset, args, logger, device):
    ...
    # --- 打印表头 ---
    header_fmt = "{:<15} | {:<10} {:<10} {:<10} | {:<10} {:<10} {:<10} {:<10}"
    separator = "-" * 105
    logger.info(separator)
    logger.info(
        header_fmt.format("Category", "img_auroc", "img_ap", "img_f1", "pix_auroc", "pix_ap", "pix_f1", "pix_pro"))
    logger.info(separator)
    ...
    for category in test_dataset.categories:
        ...
        if args.vis != 0:
            ...
            # 打印单行表格
            logger.info(get_res_str(category_res, category))

    if args.vis == 0:
        logger.info(separator)
        # 计算平均值 (确保 cal_average_res 能处理嵌套字典)
        average_res = cal_average_res(total_res)
        # 打印平均行
        logger.info(get_res_str(average_res, "Average"))
        logger.info(separator + "\n")
